...
export function useTournamentManagement(tournamentId: string) {
  ...
  const stats: TournamentStats = useMemo(() => {
    const fieldsWithReferees = fields.filter(
      (f) => f.fieldReferees && f.fieldReferees.length > 0
    );
    const fieldsWithHeadReferee = fields.filter(
      (f) => f.fieldReferees && f.fieldReferees.some((fr) => fr.isHeadRef)
    );
    const allRefereeIds = new Set(
      fields.flatMap((f) =>
        f.fieldReferees ? f.fieldReferees.map((fr) => fr.userId) : []
      )
    );
    ...
    return {
      ...
      averageRefereesPerField:
        fields.length > 0 ? totalRefereeAssignments / fields.length : 0,
    };
  }, [stages, fields]);
