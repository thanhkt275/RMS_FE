...
interface IEnhancedRouteProtector extends IRouteProtector {
  getFeaturePermission(
    pathname: string
  ): { feature: keyof typeof PERMISSIONS; action: string } | null;
...
class EnhancedRouteProtectorService implements IEnhancedRouteProtector {
  ...
  getRequiredRoles(pathname: string): readonly UserRole[] | null {
    ...
    if (config.feature && config.action) {
      return PermissionService.getAuthorizedRoles(
        config.feature,
        config.action
      );
    }
  ...
  private findMatchingRoute(pathname: string): RouteConfig | null {
    ...
    const matchingPath = Object.keys(ENHANCED_PROTECTED_ROUTES).find(
...
class RouteProtector implements IRouteProtector {
  ...
  getRequiredRoles(pathname: string): readonly UserRole[] | null {
    ...
    const matchingRoute = Object.entries(this.protectedRoutes).find(
      ([routePattern]) => pathname.startsWith(routePattern)
    );
...
export async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  if (!routeProtector.isRouteProtected(pathname)) {
    console.log(
      "[Middleware Debug] Route not protected, allowing access:",
      pathname
    );
  ...
  console.log(
    "[Middleware Debug] Token value (first 20 chars):",
    token?.substring(0, 20)
  );

  if (!token) {
    console.log(
      "[Middleware Debug] No token found, redirecting to access denied"
    );
  ...
  if (!hasPermission) {
    ...
    if (featurePermission) {
      console.log(
        `[RBAC] Feature-based access denied: ${featurePermission.feature}.${featurePermission.action} for user role: ${userRole}`
      );
    }
  ...
  if (process.env.NODE_ENV === "development") {
    const featurePermission = routeProtector.getFeaturePermission(pathname);
    console.log(
      `[RBAC] Access granted to ${pathname} for user role: ${userRole}${
        featurePermission
          ? ` (${featurePermission.feature}.${featurePermission.action})`
          : ""
      }`
    );
  }
